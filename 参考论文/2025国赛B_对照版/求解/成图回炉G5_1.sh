...
export MPLCONFIGDIR="$ROOT/日志/G5返工图1再交接/matplotlib"
...
def event(problem, attempt, observation):
    ...
    with (root / '交接/.实验记录.lock').open('a') as lock:
        ...
        temporary = target.with_suffix('.G5图.tmp')
...
try:
    ...
    event(0, 'G5图1批次成图数据预检', str(error))
...
mpl.rcParams.update({'font.sans-serif': ['FandolHei', 'Noto Sans CJK SC', 'DejaVu Sans'],
                     ...
                     'mathtext.fontset': 'stix', 'mathtext.default': 'it', 'mathtext.fallback': 'stix'})
...
for problem, name in jobs:
    ...
    try:
        ...
        with Image.open(image) as rendered:
            ...
            if name == '三路线误差按情景分布' and summary['正式分组汇总'] != json.loads(
    ...
    event(problem, f'G5图1重绘{name}', record['状态'] + '；' + record.get('原因', ''))

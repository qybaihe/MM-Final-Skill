...
状态 = 调度器.状态机(状态文件)
if 续跑 and 状态.加载():
    T_START = 状态.数据.get("起始时间", time.time())
    续跑生效 = True
else:
    T_START = time.time()
    状态.数据["起始时间"] = T_START
...
which xelatex && echo 就绪''', files=files, timeout_s=560)
    ...
    def 跑一问(编号, 重算=False):
        ...
        红结论 = 红队复算(编号, 标) if (配置["开红队"] and budget_ok()) else "未启用"
    ...
    def 红队复算(编号, 标=""):
        ...
        if 应改建模 and budget_ok():
            ...
            wave([("建模师.md", f"【仲裁返工】问题{编号}：仲裁裁定建模侧有误，见 交接/仲裁_问题{编号}.json 与 交接/返工单_问题{编号}.md。"
                   f"逐条修 {qdir}/求解_问题{编号}.py 与建模笔记，追加实验记录。只改不跑。完成写 日志/仲裁返工_问题{编号}{标}.done", f"仲裁返工_问题{编号}{标}")], timeout=1100)
    ...
    def G2门(编号):
        ...
        def _返工(明细):
            wave([("建模师.md", f"正确性门 G2（问题{编号}）未通过：{json.dumps(明细, ensure_ascii=False)[:1800]}。"
                   f"逐条修 求解/问题{编号}/求解_问题{编号}.py、交接/建模笔记_问题{编号}.md、交接/假设台账_问题{编号}.json（检验结果必须真实非空并引用结果键）。只改不跑。完成写 日志/G2返工_问{编号}.done", f"G2返工_问{编号}")], timeout=1100)

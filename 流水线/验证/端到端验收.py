...
if st:
    ...
    t0 = st.get("起始时间")
    机时 = st.get("累计机时")
    # 口径必须与驱动一致：预算按**实际在跑的机时**算，而不是首次启动至今的墙钟。
    # 驱动侧已按此口径落盘 累计机时（见 施工日志 M4-4.8）；验收若仍用墙钟，
    # 中途停跑分析/改代码的间隔会被计入——实测把 7.0h 的真实机时报成 89.8h。
    if isinstance(机时, (int, float)) and 机时 > 0:
        时长, 口径 = 机时 / 3600, "累计机时"
    elif t0:
        时长, 口径 = (time.time() - t0) / 3600, "墙钟(无累计机时可用)"
    else:
        时长, 口径 = None, ""
    记("硬指标", "总时长 ≤14h", (时长 is not None and 时长 <= 14),
      f"用时={时长:.1f}h（{口径}）" if 时长 is not None else "无起始时间")
    门 = st.get("问题门状态") or {}

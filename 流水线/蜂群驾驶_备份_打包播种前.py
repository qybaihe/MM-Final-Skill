...
def 播种一次(files=None):
        ...
        if 镜像目录.exists():
            for p in 镜像目录.rglob("*"):
                if not p.is_file() or p.stat().st_size >= 8_000_000:
                    continue
                # 角色腿的原始输出日志（日志/*.log）体积极大——全目录 104MB，
                # 单个可达 11MB——而回灌毫无价值：下游只读 执行_*.log（实测 ≤1KB）
                # 与 *.done 标记。实测（20260831）含 2.1MB 仲裁日志的批会稳定触发
                # 服务端 422，是播种反复失败的元凶。排除后 732→656 文件、111→30MB、
                # 分批数 53→18，播种一轮从 ~14 分钟降到 ~5 分钟。
                if p.suffix == ".log" and p.stat().st_size > 200_000:
                    continue
                files.append(h.f_local(str(p.relative_to(镜像目录)), p))
    # 分批上传：实测（20260831，全新空容器）上传耗时与**文件数**线性相关，约 0.7s/文件，
    # 与总大小几乎无关（80 文件/16KB 用 53s；40 文件/4MB 用 37s）。一次性播 734 文件需 ~510s，
    # 会直接撞穿 T_播种 被掐断——现象是 exec 返回空 stdout、容器仍然全空，而日志只报"缺蜂巢"。
    # 按批送既能规避超时，也让单批失败只需重试该批。
    # 双约束分批：文件数管住单批耗时（~0.7s/文件），字节数管住 payload
    # （实测批次大小从 0.2MB 到 27MB 不等，只按文件数切会切出巨批）。
    def _体积(f):
        return len(f.get("contentB64") or f.get("content") or "")

    批们, 当前, 当前字节 = [], [], 0
    for f in files:
        v = _体积(f)
        if 当前 and (len(当前) >= 60 or 当前字节 + v > 3_500_000):
            批们.append(当前)
            当前, 当前字节 = [], 0
        当前.append(f)
        当前字节 += v
    ...
    落, 首指纹, 失败批 = "", "", 0
    批数 = len(批们)
    for 序, 批 in enumerate(批们, 1):
        fp = ""
        for 试 in (1, 2, 3):
            # 实测见过服务端偶发 `RETRY: skill write 422`（同一轮里 5.2MB 的批失败、
            # 12.7MB 的批却成功），属瞬时错误而非确定性限制——就地重试即可自愈，
            # 不必让 ensure_hive 走全量复播（那要十来分钟）。
            d = h.exec("mkdir -p 日志 任务 交接 求解 论文 审稿 pylibs; "
                       "printf '%s@%s' \"$(hostname | cut -c1-12)\" "
                       "\"$(cut -c1-8 /proc/sys/kernel/random/boot_id 2>/dev/null)\"",
                       files=批, timeout_s=T_播种, quiet=True)
            fp = (d.get("stdout") or "").strip()
            if fp:
                break
            if 试 < 3:
                time.sleep(3)
        if not fp:
            失败批 += 1
            log(f"!! 播种第 {序}/{批数} 批 3 次均未落地（{len(批)} 文件）")
            continue
        落 = fp[-21:]
        # 全量播种要跨十来分钟（~0.7s/文件），中途容器轮换会让先前批次白传。
        # 这里只负责**让漂移可见**：真正的兜底是 ensure_hive 的探活循环——
        # 它会在播种后继续探，探到 missing 会再播（而新的"连续两次同一空实例"
        # 门槛又能挡住为临时访客白播）。
        首指纹 = 首指纹 or 落
        if 落 != 首指纹:
            log(f"!! 播种中途实例漂移 {首指纹} → {落}（第 {序}/{批数} 批），"
                f"先前批次可能已失效，交由 ensure_hive 复播")
            首指纹 = 落
    if 失败批:
        log(f"!! 播种有 {失败批}/{批数} 批未落地（交由 ensure_hive 复播）")
    # wheels 安装与文件上传解耦：pip 装 7 个 wheel（55MB pylibs）是另一个量级的开销，
    # 混在上传批里会让每一批都背上它的超时风险。
    d = h.exec("chmod +x bin/role.sh 2>/dev/null; "
               "([ -d pylibs/sklearn ] || pip3 install --quiet --no-index --no-deps --no-compile "
               "--target=/tmp/蜂巢/pylibs 运行时/wheels/*.whl); printf '播种于 %s' \"$(hostname)\"",
               timeout_s=T_播种, quiet=True)
    return (d.get("stdout") or "").strip()[-14:] or 落

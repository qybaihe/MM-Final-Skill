...
# ---------------- 超时分级（实测标定，见 施工日志 M4-4.9）----------------
# 教训：32 处 h.exec 有 28 处未传 timeout_s，吃 hive_sdk 默认的 560s。而探活实测仅
# 3.0-3.8s、上传 8MB 实测 9.1s——秒级操作配了 9.3 分钟的等待预算。于是每次网关抖动
# 都白等满超时，ensure_hive（探活↔播种交替最多 4 轮）最坏 4×(560+560)≈75 分钟。
# 实测第二次全开跑：22.4h 里 13.16h（58.9%）耗在这类等待上，而真正跑腿只有 5.92h。
T_探活 = 20      # 探活：实测 3.0-3.8s，留 5 倍余量
T_轻 = 30        # 轮询 done / 读单行 / 删文件 / find 列清单：均为秒级操作
T_种子 = 420     # 单包播种：上传+解压实测 9.2s，但同请求里还要 pip 装 7 个 wheel
T_播种 = 300     # 全量镜像上传（实测 8MB≈9s）+ pip 装 7 个 wheel（55MB，真正的大头）
    ...
    while pending and attempt < 2:
        ...
        for 试 in range(3):
            ...
            for role, task, name in pending:
                h.exec(f"rm -f 日志/{name}.done", quiet=True, timeout_s=T_轻)
            ...
            d = h.exec(f"c=0; for f in {名单}; do [ -f $f ] && c=$((c+1)); done; echo $c; "
                       ...
                       f"\"$(cut -c1-8 /proc/sys/kernel/random/boot_id 2>/dev/null)\"", quiet=True, timeout_s=T_轻)
...
def run_script(script_path, log_name, budget=1320, repair_role="建模师.md", repairs=2):
    ...
    while attempt <= repairs:
        attempt += 1
        ensure_hive()
        h.exec(f"rm -f 日志/{log_name}.done; nohup bash -c '{执行器} {script_path} > 日志/{log_name}.log 2>&1; echo rc=$? > 日志/{log_name}.done' >/dev/null 2>&1 & echo started", quiet=True, timeout_s=T_轻)
        t0 = time.time()
        rc = None
        轮换了 = False
        while time.time() - t0 < budget:
            ...
            d = h.exec(f"cat 日志/{log_name}.done 2>/dev/null; "
                       ...
                       f"pgrep -f '{script_path}' >/dev/null && echo RUNNING", quiet=True, timeout_s=T_轻)
            out = (d.get("stdout") or "")
            见活 = "HIVE_ALIVE" in out
            在跑 = "RUNNING" in out
            rc = None
...
def checkpoint_dir(remote_dir, tag):
    """收割整个远端目录（按文件清单）。"""
    d = h.exec(f"find {remote_dir} -type f 2>/dev/null | head -80", quiet=True, timeout_s=T_轻)
...
def 基建清单():
    """L0 基座层：一轮内**不变**的资产 —— [(容器内相对路径, 本地Path)]。

    分层的意义见 契约清单() 的注释：上行只该带 L0+L1，L2（腿的原始输出）
    只回传不回灌。这里返回「路径对」而非 h.f_local 字典，是为了能打成
    单个 tar 包上传（见 打包上传()）。
    """
    ...
    # 内部术语.txt 必须在列：漏了的话容器里 审计.py 的 try/except 会静默拿到空词表，
    ...
    for w in 轮子目录.glob("*.whl"):
        对.append((f"运行时/wheels/{w.name}", w))
...
def 基建文件():
    """L0 基座的逐文件形式（保留给仍按 files= 传参的调用方）。"""
    return [h.f_local(远, 本) for 远, 本 in 基建清单() if 本.exists()]


# L1 契约层：镜像目录里除「腿的原始输出」以外的一切都回灌。
# 这里刻意用**黑名单**而不是白名单——白名单要求穷举每一个消费方，而这个前提
# 不成立：初版白名单就漏掉了 求解/**/图片/*.png（26 张，xelatex 编译必需，
# 漏了直接编译失败）和 论文/*.pdf（S5 审稿与出版要读）。排除面越小，
# 要枚举正确的东西越少，回归风险越低。
腿日志上限 = 50_000       # 超过此值的 .log 视为腿原始输出
单文件上限 = 8_000_000    # 单个巨型文件不进包


def 契约清单():
    """L1 契约层：镜像目录里**回灌确有意义**的文件 —— [(相对路径, 本地Path)]。

    为什么要排（破环）：镜像目录同时是**下行落地区**和**上行源**，
    此前播种把它整个回灌，于是形成正反馈膨胀环——
    腿跑一轮 → 回传 → 镜像变大 → 下次播种更慢 → 超过实例寿命 → 播种失败 →
    再播更慢。实测分批数 18→56→60、单轮 ~900s，而实例漂移中位周期只有
    ~300s（实测间隔 151/256/647/332/262/401/167s）：播种耗时是实例寿命的
    3 倍，数学上不可能原子完成，日志里"播种中途实例漂移…先前批次可能已失效"
    就是这么来的。

    排什么：只排腿的原始输出日志。实测（20260831）镜像 769 文件 / 145.8MB 中，
    102 个 >50KB 的 .log 独占 114.1MB——膨胀 100% 集中在这一处，而它们在容器内
    **没有任何消费方**：门检.py:103 与 wave 轮询只读 日志/*.done（126 个共
    33.8KB），脚本诊断只读 执行_*.log（实测 ≤1KB）。排掉后 667 文件 / 31.8MB，
    求解图片 26 / 正文 15 / 论文PDF 9 / done 标记 126 / 交接契约 28 全部保住。
    """
    对 = []
    if not 镜像目录.exists():
        return 对
    for p in 镜像目录.rglob("*"):
        ...
        if 大小 >= 单文件上限:
            continue
        if p.suffix == ".log" and 大小 > 腿日志上限:
            continue
        对.append((str(p.relative_to(镜像目录)), p))
    return 对


def 打包上传(对列表, 脚本, timeout_s=None, 说明=""):
    """把一批本地文件打成**单个 tar.gz** 上传并就地解开，再跑 脚本。

    实测（20260831）hive 上传耗时 ∝ **文件数**（~0.7s/文件），与总体积几乎
    无关。全量种子 523 文件 / 22.8MB：逐文件预估 366s，打成 17.3MB 的包后
    上传+解压只要 **9.2s（39.9×）**，522/523 落地（唯一差额是 论文/format.cls
    在 L0 与 L1 重名，已按"基座优先"去重）。

    比"快"更要紧的是**原子**：逐文件播种要跨十几分钟，中途必然撞上实例漂移，
    先前批次全部白传；单请求则要么整体落地、要么整体没落地，不存在半播状态。
    9.2s ≪ 300s 漂移周期，播种这才在数学上可能收敛。

    这个模式本仓库早已验证过——即时回传（下行）就是 tar+base64 增量包，
    注释写着"而不是每个文件一次 harvest（后者在多实例摇摆下极易漏文件且慢）"。
    播种（上行）此前一直逐文件，是同一个病只治了一半。
    """
    # 去重：同一容器路径可能既来自 L0 基座又来自镜像里的旧副本。**基座优先**——
    # 镜像副本是上一轮回传下来的快照，若让它覆盖，本地刚改过的 bin/门检.py、
    # 角色/*.md 会被悄悄回退成旧版。
    见 = {}
    for 远, 本 in 对列表:
        if 远 not in 见 and pathlib.Path(本).is_file():
            见[远] = 本
    buf = io.BytesIO()
    with tarfile.open(fileobj=buf, mode="w:gz") as tf:
        for 远, 本 in 见.items():
            try:
                tf.add(str(本), arcname=远)
            except OSError:
                pass          # 打包途中本地文件被删/改，跳过即可，不该中断播种
    包 = buf.getvalue()
    临时包 = 产出目录.parent / "_种子.tar.gz"
    临时包.write_bytes(包)
    try:
        # `|| exit 9`：解压失败必须立刻中止。否则后续脚本照跑、末尾 printf 照样
        # 吐出指纹，调用方会把"包根本没落地"误判成播种成功。
        d = h.exec("tar -xzf _种子.tar.gz || exit 9\nrm -f _种子.tar.gz\n" + 脚本,
                   files=[h.f_local("_种子.tar.gz", 临时包)],
                   timeout_s=timeout_s or T_播种, quiet=True)
    finally:
        临时包.unlink(missing_ok=True)
    if 说明:
        log(f"{说明}：{len(见)} 文件 / 包 {len(包)/1e6:.1f}MB / {d.get('ms', '?')}ms"
            f" exit={d.get('exit')}")
    return d
...
def 蜂巢就绪(强制播种=False):
    ...
    d = h.exec("printf '%s|%s|' \"$(hostname)\" \"$(cat /proc/sys/kernel/random/boot_id 2>/dev/null)\"; "
               # 校验必须覆盖"腿真正跑得起来"所需的全部要件，而不只是两个脚本。
               # 教训升级（20260831）：手工播种漏了 数据/ 与 文献卡片库.md，而旧判据
               # 只查 AGENTS.md+role.sh，于是"蜂巢看起来在、实际残缺"探不出来——
               # 腿会读不到附件、G5 也会因缺文献卡片库而挂。宁可判 missing 触发补播。
               "[ -f AGENTS.md ] && [ -f bin/role.sh ] && [ -s 运行时/文献卡片库.md ] "
               "&& [ -n \"$(ls 数据/* 2>/dev/null)\" ] && [ -n \"$(ls 角色/*.md 2>/dev/null)\" ] "
               "&& echo alive || echo missing",
               quiet=True, timeout_s=T_探活)
    out = (d.get("stdout") or "").strip()
    if "|" not in out:
        return "unknown", "?"
    ...
    指纹 = f"{主机[:12]}@{boot[:8]}"
    return ("alive" if 态.strip() == "alive" else "missing"), 指纹
...
def 播种一次(对列表=None):
    """把 L0 基座 + L1 契约打成**一个包**播到当前路由到的实例。返回该实例指纹。

    为什么是"一个包"而不是分批：实测 523 文件 / 22.8MB 逐文件要 366s，
    打成 17.3MB 的 tar.gz 后上传+解压只要 9.2s（39.9×，见 打包上传() 注释）。
    但真正的收获不是快，是**原子**——旧实现要跨十几分钟，而实例漂移中位周期
    只有 ~300s，于是"播到一半换了实例、先前批次白传"是必然而非偶然。
    9.2s ≪ 300s 之后，播种才第一次在数学上可能收敛。

    随之删掉的是分批播种的全套配套设施：双约束切批、逐批三试重试、
    "播种中途实例漂移"的可见性日志。它们都在为一个不该存在的问题服务。

    wheels 安装也并入同一请求。旧实现把它拆出去，是因为当时有十几个上传批、
    每批都要独立背超时风险；现在只剩一个请求，合并反而修掉一个真实缺陷——
    此前 pip 那一发可能落到与文件**不同的实例**上，装完即废。
    """
    对 = 对列表 if 对列表 is not None else (基建清单() + 契约清单())
    脚本 = ("mkdir -p 日志 任务 交接 求解 论文 审稿 pylibs\n"
            "chmod +x bin/role.sh 2>/dev/null\n"
            "[ -d pylibs/sklearn ] || pip3 install --quiet --no-index --no-deps "
            "--no-compile --target=/tmp/蜂巢/pylibs 运行时/wheels/*.whl\n"
            "printf '%s@%s' \"$(hostname | cut -c1-12)\" "
            "\"$(cut -c1-8 /proc/sys/kernel/random/boot_id 2>/dev/null)\"")
    for 试 in (1, 2, 3):
        # 服务端偶发 `RETRY: skill write 422`（实测同一轮里 5.2MB 的批失败、
        # 12.7MB 的批却成功），属瞬时错误而非确定性限制，就地重试即可自愈。
        d = 打包上传(对, 脚本, timeout_s=T_种子, 说明=f"播种第{试}发")
        落 = (d.get("stdout") or "").strip()
        if 落:
            return 落[-21:]
        if 试 < 3:
            time.sleep(3)
    log("!! 播种 3 发均未落地（交由 ensure_hive 复播）")
    return ""
...
def ensure_hive(最多播种=4):
    ...
    末指纹 = "?"
    未知次数 = 0
    待播确认 = {}      # 指纹 -> 连续探到 missing 的次数（防为"临时访客"实例白播）
    总轮 = 0
    while True:
        总轮 += 1
        if 总轮 > 20:
            log(f"!! ensure_hive 探活 {总轮} 轮仍未稳定（实例池 {sorted(实例池)}）——继续推进")
            return False, 末指纹
        态, 指纹 = 蜂巢就绪()
        末指纹 = 指纹
        if 态 == "alive":
            ...
            continue
        if 态 == "unknown":
            # 「没问到」≠「确定为空」：只重试探活，绝不据此播种。
            # 旧实现把两者都折叠成 False，而本函数对 False 的反应是立刻全量播种，
            # 于是一次网关抖动就触发和"确实是空实例"一样昂贵的补救（实测单轮可达 2 小时）。
            未知次数 += 1
            连续 = 0
            if 未知次数 >= 3:
                log(f"!! 探活连续 {未知次数} 次无响应（网关抖动），不播种、继续推进")
                return False, 指纹
            time.sleep(3)
            continue
        # 确认是空实例（missing）
        未知次数 = 0
        连续 = 0
        待播确认[指纹] = 待播确认.get(指纹, 0) + 1
        if 待播确认[指纹] < 2:
            # 实测网关约 8% 的请求会漏到一个"临时访客"实例上，它往往再也不会被路由到。
            # 要连着两次打到同一个空实例，才认为它真要承载工作、值得一次全量播种
            # （22.4MB 镜像 + 55MB 依赖）。
            continue
        if 播种次数 >= 最多播种:
...
def 图片腿(名, 角色文件, 任务文本, 图们, timeout=900, reasoning=True):
    ...
    h.exec(f"rm -f 日志/{名}.done", quiet=True, timeout_s=T_轻)
...
def 主流程():
    ...
    def _播种():
        ...
        # 首播同样走单包：57 个基建文件逐传约 40s，打包后与全量种子一样是**一次
        # 原子请求**。这里不带 契约清单()——首播的前提就是"从零重建"，镜像里
        # 若还有上一轮残留，回灌进来只会污染新一轮的初始状态。
        d = 打包上传(基建清单(), r'''mkdir -p 日志 任务 交接 求解 论文 审稿 pylibs
chmod +x bin/role.sh
...
EOF
which xelatex && echo 就绪''', timeout_s=T_种子, 说明="S0 首播")
